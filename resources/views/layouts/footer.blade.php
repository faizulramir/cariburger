                ...
                success:function(data) {
                    ...
                    if (data.data.length > 0 && data.found) {
                        for (let index = 0; index < arr_data.length; index++) {
                            ...
                            let ttData = ''
                            let if_waze = ''
                            let waze_confirm = 'Waze to ' + arr_data[index].name + '@' + arr_data[index].city + '?';
                            var b = waze_confirm.replace(/'/g, '/');
                            let waze_url = 'https://waze.com/ul?ll=';
                            if_waze = arr_data[index].lat && arr_data[index].lng ? `
                                        <a href="${waze_url + arr_data[index].lat + ',' + arr_data[index].lng + '&z=10'}" target="_blank" style="text-decoration: none" onclick="return confirm('${b}')">
                                            <i>
